  ...
  Future<void> _fetchRemoteMonitoringData(String period) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _periodData[period] = {
            'periodLabel': data['periodLabel'] ?? 'Hari Ini',
            'summary': data['summary'] ?? '',
            'emotionalCenter': {
              'status': data['emotionalCenter']?['status'] ?? 'Baik',
              'level': data['emotionalCenter']?['level'] ?? 3,
              'gradientColors': [const Color(0xFFFFF8E1), const Color(0xFFFFE082)],
              'textColor': const Color(0xFFF57F17),
              'description': data['emotionalCenter']?['description'] ?? '',
            },
            'risks': (data['risks'] as List<dynamic>? ?? []).map((r) => {
              'name': r['name'] ?? 'Risiko',
              'type': r['type'] ?? 'stress',
              'percent': (r['percent'] as num?)?.toDouble() ?? 0.5,
              'levelLabel': r['levelLabel'] ?? 'Sedang',
              'color': const Color(0xFFD32F2F),
              'badgeBg': const Color(0xFFFFDCDD),
            }).toList(),
            'xLabels': List<String>.from(data['xLabels'] ?? []),
            'chartData': (data['chartData'] as List<dynamic>? ?? [])
