...
\begin{tikzpicture}[remember picture, overlay]
  \node[inner sep=0pt, at={(current page.center)}] {
    \includegraphics[width=\paperwidth,height=\paperheight]{images/cover-fix.png}
  };
  
  % Top Left: POLINEMA Logo
  \node[anchor=north west, inner sep=0pt] at ($(current page.north west) + (2.0cm, -1.8cm)$) {
    \includegraphics[height=1.6cm, keepaspectratio]{images/logo_polinema.png}
  };
  
  % Top Right: HOLOGY 9.0 Logo
  \node[anchor=north east, inner sep=0pt] at ($(current page.north east) + (-2.0cm, -1.8cm)$) {
    \includegraphics[height=1.6cm, keepaspectratio]{images/logo-hology-black.png}
  };
\end{tikzpicture}
\null
\end{titlepage}
...
\input{sections/01_title_abstract.tex}
\input{sections/02_latar_belakang.tex}
\input{sections/03_tujuan_manfaat.tex}
\input{sections/04_fitur_aplikasi.tex}
\input{sections/05_metode_pengembangan.tex}
\input{sections/06_analisis_desain.tex}
\input{sections/07_arsitektur_sistem.tex}
\input{sections/08_mockup.tex}
\input{sections/09_rencana_implementasi.tex}
\input{sections/11_daftar_pustaka.tex}
\input{sections/10_lampiran.tex}
