...
\hline
\rowcolor{TableAltRowBg}
Speech Emotion Recognition (FunASR) & Mengekstraksi kondisi afektif dan emosi suara secara akustik \\
\hline
Speech-to-Text (OpenAI STT) & Mengubah suara pengguna menjadi teks transkripsi \\
\hline
\rowcolor{TableAltRowBg}
Text-to-Speech (OpenAI TTS) & Menghasilkan respons suara sintesis yang natural \\
\hline
Safety Policy Engine (SafetyGate) & Menentukan izin eksekusi (\textit{ResponseMode} \& \textit{LLMMode}) serta mengisolasi RAG \\
\hline
\rowcolor{TableAltRowBg}
Embedding Engine (FastEmbed) & Menghasilkan representasi vektor 384-dimensi untuk pencarian semantik \\
\hline
Vector Database (Qdrant) & Penyimpanan embedding dan pencarian semantik RAG \\
\hline
\rowcolor{TableAltRowBg}
Database (PostgreSQL) & Penyimpanan data pengguna, diary, asesmen, dan riwayat percakapan \\
\hline
Cache \& Task Queue (Redis \& ARQ) & Penyimpanan sesi dan antrean pemrosesan tugas latar belakang \\
\hline
\rowcolor{TableAltRowBg}
Long-term Memory (Memory Worker) & Menyimpan preferensi dan konteks pengguna jangka panjang \\
\hline
\end{tabularx}
\end{table}
...
\begin{enumerate}[leftmargin=*,label=\arabic*.]
    ...
    \item \textbf{Konseling Interaktif (Teks \& Suara)}: Pengguna berinteraksi dalam sesi percakapan real-time dengan agen AI berbasis CBT, didukung analisis emosi suara (\textit{FunASR emotion2vec+}).
    \item \textbf{Sintesis Jurnal \& Safety Priority Gate}: Sistem backend merangkum diari secara otomatis dan mengevaluasi status keselamatan melalui \textit{SafetyGate}. Jika terdeteksi risiko tinggi atau kritis (\textit{High / Critical Risk} berbasis WHO EASE), sistem mengaktifkan protokol krisis: mengisolasi pencarian RAG umum (\textit{Strict RAG Isolation}), menonaktifkan generasi bebas LLM (\textit{LLMMode: None}), serta menampilkan modul bantuan darurat dan kontak profesional/hotline.
\end{enumerate}
